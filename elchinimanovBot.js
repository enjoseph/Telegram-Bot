...
// Bu funksiya botun hansi chatlarda oldugunu database de qeyd edir
bot.onText(/\/quizstart/, (msg) => {
  const chatID = { isDailyQuestion: false, _id: msg.chat.id }; // chat idni alir
  setID_DB(chatID); // idleri data base yazir
...
const setID_DB = (chatID) => {
  const findIndex = chatID_DB.findIndex((item) => item._id === chatID._id);
  if (findIndex == -1) chatID_DB.push(chatID);
};
 ...
 ${selectCode(currentQuestion.id)}
  ...
  `;
    ...
    try {
      ...
      const pollID = await bot.sendPoll(chatID,questionText,answerOptions,{ is_anonymous: false,  correct_option_id: 1 });
      console.log(pollID);
    } catch (error) {
...
const isDaily = () => {
  ...
  let isDaily;
  if (hours == 8) isDaily = true;

  return isDaily;
};

setInterval(sendQuiz, 5000);

const selectCode = (codeID) => {
  const questionCode = quizDB.find((element) => element.id === codeID);
  if (questionCode) {
...
const wrongAnswers = (pollID, pollCorrectAnswer) => {
